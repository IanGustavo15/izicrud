...
const props = defineProps<{
    itens: {
        data: { id: number; id_inscricao: number; pontuacao_total: number; acertos: number; erros: number; tempo_total_minutos: number; percentual_acerto: number }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; id_inscricao: number; pontuacao_total: number; acertos: number; erros: number; tempo_total_minutos: number; percentual_acerto: number }[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/resultado/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('Resultado excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o resultado.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/resultado', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar resultados..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>

                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead class="cursor-pointer" @click="toggleSort('id_inscricao')">Inscricao<span v-if="sortColumn === 'id_inscricao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('pontuacao_total')">Pontuacao total<span v-if="sortColumn === 'pontuacao_total'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('acertos')">Acertos<span v-if="sortColumn === 'acertos'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('erros')">Erros<span v-if="sortColumn === 'erros'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('tempo_total_minutos')">Tempo total em Minutos<span v-if="sortColumn === 'tempo_total_minutos'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('percentual_acerto')">Percentual de Acerto<span v-if="sortColumn === 'percentual_acerto'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="w-1/6">Ações</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            <TableCell>{{ item.id_inscricao }}</TableCell>
                            <TableCell>{{ item.pontuacao_total }}</TableCell>
                            <TableCell>{{ item.acertos }}</TableCell>
                            <TableCell>{{ item.erros }}</TableCell>
                            <TableCell>{{ item.tempo_total_minutos }}</TableCell>
                            <TableCell>{{ item.percentual_acerto }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/resultado/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} resultados
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este resultado? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
