...
const props = defineProps<{
    itens: {
        data: { id: number; id_simulado: number; id_users: number; pontuacao_final: number; posicao_rank: number; classificacao: boolean }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; id_simulado: number; id_users: number; pontuacao_final: number; posicao_rank: number; classificacao: boolean }[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/rank/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('Rank excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o rank.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/rank', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar ranks..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>

                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead class="cursor-pointer" @click="toggleSort('id_simulado')">Simulado<span v-if="sortColumn === 'id_simulado'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('id_users')">Usuario<span v-if="sortColumn === 'id_users'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('pontuacao_final')">Pontuação Final<span v-if="sortColumn === 'pontuacao_final'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('posicao_rank')">Posição no Rank<span v-if="sortColumn === 'posicao_rank'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('classificacao')">Classificação<span v-if="sortColumn === 'classificacao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="w-1/6">Ações</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            <TableCell>{{ item.id_simulado }}</TableCell>
                            <TableCell>{{ item.id_users }}</TableCell>
                            <TableCell>{{ item.pontuacao_final }}</TableCell>
                            <TableCell>{{ item.posicao_rank }}</TableCell>
                            <TableCell>{{ item.classificacao }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/rank/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} ranks
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este rank? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
