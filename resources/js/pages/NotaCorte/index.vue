...
const props = defineProps<{
    itens: {
        data: { id: number; id_simulado: number; valor_corte: number }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; id_simulado: number; valor_corte: number }[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/notacorte/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('NotaCorte excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o notacorte.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/notacorte', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar notacortes..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('valor_corte')">Valor de Corte<span v-if="sortColumn === 'valor_corte'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            ...
                            <TableCell>{{ item.valor_corte }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/notacorte/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} notacortes
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este notacorte? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
