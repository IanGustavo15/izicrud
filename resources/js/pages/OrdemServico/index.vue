...
const props = defineProps<{
    itens: {
        data: { id: number; id_servico: number; id_moto: number; data_servico: string; realizado: boolean; moto?: {nome:string}; servico?: {descricao:string} }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    ...
    allItens: {
        moto: any;
        servico: any; id: number; id_servico: number; id_moto: number; data_servico: string; realizado: boolean
}[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/ordemservico/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('OrdemServico excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o ordemservico.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/ordemservico', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar ordemservicos..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('id_moto')">Moto<span v-if="sortColumn === 'id_moto'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('data_servico')">Realizado em<span v-if="sortColumn === 'data_servico'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('realizado')">Realizado<span v-if="sortColumn === 'realizado'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="w-1/6">Ações</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            <TableCell>{{ item.servico?.descricao }}</TableCell>
                            <TableCell>{{ item.moto?.modelo }}</TableCell>
                            <TableCell>{{ formatDate(item.data_servico) }}</TableCell>
                            <TableCell>{{ item.realizado ? 'Sim' : 'Não' }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/ordemservico/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} ordemservicos
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este ordemservico? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
