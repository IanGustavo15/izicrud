...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Pecas', href: '/peca' },
];

const headerTitle = 'Pecas';
const headerDescription = 'Gerencie seus pecas aqui.';

const props = defineProps<{
    itens: {
        data: { id: number; descricao: string; codigo_unico: string; preco_de_custo: number; preco_de_venda: number; quantidade: number; estoque: number }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; descricao: string; codigo_unico: string; preco_de_custo: number; preco_de_venda: number; quantidade: number; estoque: number }[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/peca/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('Peca excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o peca.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/peca', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar pecas..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>

                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead class="cursor-pointer" @click="toggleSort('descricao')">Descrição<span v-if="sortColumn === 'descricao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('codigo_unico')">Código Único<span v-if="sortColumn === 'codigo_unico'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('preco_de_custo')">Preço de Custo<span v-if="sortColumn === 'preco_de_custo'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('preco_de_venda')">Preço de Venda<span v-if="sortColumn === 'preco_de_venda'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('quantidade')">Quantidade<span v-if="sortColumn === 'quantidade'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('estoque')">Estoque<span v-if="sortColumn === 'estoque'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            ...
                            <TableCell>{{ item.codigo_unico }}</TableCell>
                            <TableCell>R$ {{ item.preco_de_custo  }}</TableCell>
                            <TableCell>R$ {{ item.preco_de_venda }}</TableCell>
                            <TableCell>{{ item.quantidade }}</TableCell>
                            <TableCell>{{ item.estoque }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/peca/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} pecas
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este peca? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
