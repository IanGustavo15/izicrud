<script setup lang="ts">
    ...
    const props = defineProps < {
        item ? : {
            id: number;id_cliente: number;placa: string;modelo: string;ano: number;tipo: number
        };
        sidebarNavItems: {
    ...
    const form = useForm({
        ...
        ano: props.item?.ano.toString() || 2025,
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription"
        ...
        <div class="space-y-6">
            <HeadingSmall :title="isEditing ? 'Editar Veículo' : 'Criar Novo Veículo'"
                description="Gerencie os detalhes do veículo" />
        </div>
                ...
                <div class="flex flex-col gap-4 p-4">
                    ...
                    <form @submit.prevent="submitForm" class="space-y-6">
                        ...
                        <div>
                            ...
                            <Input id="ano" v-model.number="form.ano" type="number" step="1"
                                placeholder="Digite Ano" min="1" max="2026" />
                        </div>
                        <div>

                            <Label for="tipo">Tipo</Label>
                            <RadioGroup default-value="option-one" v-model="form.tipo">
                                <div class="flex items-center space-x-2">
                                    <RadioGroupItem id="option-one" value="1" />
                                    <Label for="option-one">Carro</Label>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <RadioGroupItem id="option-two" value="2" />
                                    <Label for="option-two">Moto</Label>
                                </div>
                            </RadioGroup>

                        </div>
                        <Button type="submit" class="my-4" :disabled="form.processing">
                            {{ isEditing ? 'Atualizar Veículo' : 'Criar Veículo' }}
                        </Button>
                    </form>
                </div>
            </div>
