...
const props = defineProps<{
    item?: { id: number; modelo: string; placa: string; id_cliente: number };
...
function submitForm() {
    if (isEditing.value) {
        form.put(`/moto/${props.item?.id}`, {
            onSuccess: () => {
                showAlert('Moto atualizado com sucesso!', 'success');
                formErrors.value = {};
            },
            onError: (errors) => {
                formErrors.value = errors as unknown as Record<string, string[]>;
                const errorMessages = Object.values(formErrors.value).flat().join(', ');
                showAlert(`Erro ao atualizar o moto: ${errorMessages}`, 'destructive');
            },
        });
    } else {
        form.post('/moto', {
            onSuccess: () => {
                showAlert('Moto criado com sucesso!', 'success');
                formErrors.value = {};
            },
            onError: (errors) => {
                formErrors.value = errors as unknown as Record<string, string[]>;
                const errorMessages = Object.values(formErrors.value).flat().join(', ');
                showAlert(`Erro ao criar o moto: ${errorMessages}`, 'destructive');
            },
        });
    }
}
...
<template>
    <Head :title="isEditing ? 'Editar Moto' : 'Criar Moto'" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="space-y-6">
            <HeadingSmall :title="isEditing ? 'Editar Moto' : 'Criar Novo Moto'" description="Gerencie os detalhes do moto" />
        </div>
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="flex flex-col gap-4 p-4">
                    <h2 class="text-lg font-semibold">{{ isEditing ? 'Editar Moto' : 'Criar Novo Moto' }}</h2>
                ...
                <div>
                    ...
                    <Input id="placa" v-model="form.placa" type="text" placeholder="Digite Placa" />
                </div>
                <div>
    <div>
                    <Label for="id_cliente">Proprietario</Label>
                    <Select v-model="form.id_cliente">
                        ...
                        <Button type="submit" class="my-4" :disabled="form.processing">
                            {{ isEditing ? 'Atualizar Moto' : 'Criar Moto' }}
                        </Button>
                    </form>
                </div>
            </div>
