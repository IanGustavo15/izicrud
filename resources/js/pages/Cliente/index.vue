...
const props = defineProps<{
    itens: {
        data: { id: number; nome: string; cpf: string; contato: string }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; nome: string; cpf: string; contato: string }[];
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('contato')">Contato<span v-if="sortColumn === 'contato'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
