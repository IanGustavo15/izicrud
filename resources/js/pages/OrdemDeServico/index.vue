...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Ordens de Serviço', href: '/ordemdeservico' },
];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/ordemdeservico/${itemToDelete.value}`, {
            ...
            onError: () => {
                showAlert('Erro ao excluir a Ordem de Serviço.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar Ordens de Serviço..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('data_de_saida')">Data de Saída<span v-if="sortColumn === 'data_de_saida'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('valor_total')">Valor Total<span v-if="sortColumn === 'valor_total'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('observacao')">Observação<span v-if="sortColumn === 'observacao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            ...
                            <TableCell>R$ {{ item.valor_total }}</TableCell>
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} Ordens de Serviço
                        </p>
                    </div>
