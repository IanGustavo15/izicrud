...
const props = defineProps<{
    itens: {
        data: { id: number; id_inscricao: number; id_questao: number; resposta_selecionada: string; tempo_resposta_segundos: number; correta: boolean }[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
    };
    allItens: { id: number; id_inscricao: number; id_questao: number; resposta_selecionada: string; tempo_resposta_segundos: number; correta: boolean }[];
...
function deleteItem(): void {
    if (itemToDelete.value !== null) {
        router.delete(`/resposta/${itemToDelete.value}`, {
            onSuccess: () => {
                showAlert('Resposta excluído com sucesso!', 'success');
                showDeleteDialog.value = false;
                itemToDelete.value = null;
            },
            onError: () => {
                showAlert('Erro ao excluir o resposta.', 'destructive');
                showDeleteDialog.value = false;
            },
        });
    }
...
function goToPage(page: number) {
    router.get('/resposta', { page }, { preserveState: true, preserveScroll: true });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs" :headerTitle="headerTitle" :headerDescription="headerDescription" :sidebarNavItems="props.sidebarNavItems">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min">
                <div class="mb-4">
                    <input v-model="searchQuery" type="text" placeholder="Pesquisar respostas..." class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead class="cursor-pointer" @click="toggleSort('id_questao')">Questao<span v-if="sortColumn === 'id_questao'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('resposta_selecionada')">Resposta Selecionada<span v-if="sortColumn === 'resposta_selecionada'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('tempo_resposta_segundos')">Tempo para Resposta<span v-if="sortColumn === 'tempo_resposta_segundos'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                            <TableHead class="cursor-pointer" @click="toggleSort('correta')">Correta<span v-if="sortColumn === 'correta'" class="ml-2">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span></TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="(item, index) in paginatedItens" :key="index">
                            ...
                            <TableCell>{{ item.id_questao }}</TableCell>
                            <TableCell>{{ item.resposta_selecionada }}</TableCell>
                            <TableCell>{{ item.tempo_resposta_segundos }}</TableCell>
                            <TableCell>{{ item.correta }}</TableCell>
                            <TableCell>
                                <ul class="flex flex-row gap-4">
                                    <li>
                                        <Button variant="link" @click="router.visit(`/resposta/${item.id}/edit`)" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 ring-1 ring-blue-500/20">
                ...
                <div class="flex items-center justify-between px-4 py-2">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando {{ filteredItens.length }} de {{ props.itens.total }} respostas
                        </p>
                    </div>
            ...
            <Dialog v-model:open="showDeleteDialog">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tem certeza de que deseja excluir este resposta? Esta ação não pode ser desfeita.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
