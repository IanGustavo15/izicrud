...
export function useDashboardData() {
  ...
  // Computed properties para estatísticas rápidas
  const totalRevenue = computed(() => {
    return revenueChartData.value.reduce((sum, item) => sum + item.value, 0);
  });

  const averageOrderValue = computed(() => {
    const total = recentOrdersData.value.reduce((sum, order) => sum + order.total, 0);
    return total / recentOrdersData.value.length;
  });

  const activeServices = computed(() => {
    return servicesData.value.filter(service => service.status === 'ativo').length;
  });
